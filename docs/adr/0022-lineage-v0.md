...
- **状态**：已接受（v0.5 增补见文末：分支模型 + 每消费者血缘队列）
- **日期**：2026-08-27
- **决策者**：Pride Leong
...
## v0.5 增补（2026-08-28）：分支模型 + 每消费者血缘队列
...
### 2. 旁路 FIFO → 每消费者血缘队列（解除单消费者约束）

- 原方案 3 的单 FIFO 在多消费者下会互相偷取（弹出序错配）——ADR 风险表预言的第一项
- v0.5：每个 (stage, 输入通道) 一个 `LineageQueue`；`publish_bytes` 把血缘**扇出拷贝**到该通道注册的所有队列，各 stage 按自己的消费序弹出
- 正确性约束从「单消费者每通道」放宽为「每 stage 单线程消费自己的队列」；发布端多线程安全（队列内锁）
- 代价：每消费者一份拷贝（Lineage 小值对象，队列深度有界 = 2×队列深度）
